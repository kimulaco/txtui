import { prettyAlignLines } from "../features/live-editor/formatter";

const UI_EDITOR_SELECTOR = ".code-editor-wrapper[data-ui-editor]";
const EDITOR_INITIALIZED_ATTR = "data-live-editor-initialized";
const BUTTON_UPDATE_DELAY = 1200;
...
export function initUIEditorElement(
  wrapper: HTMLElement,
  originalMap: WeakMap<HTMLElement, string>,
): void {
...
export function initUIEditors(): void {
  const originalMap = new WeakMap<HTMLElement, string>();
  const wrappers = document.querySelectorAll<HTMLElement>(UI_EDITOR_SELECTOR);
...
if (typeof document !== "undefined") {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => initUIEditors(), {
      once: true,
    });
  } else {
    initUIEditors();
